...
const start = async (token,message_id,from,chat,text) => {
	...
	const r_text = `
		换汇统计命令：\n
		1.申请成为管理员：发送 <b>申请管理员</b>\n
		2.申请成为操作员：发送 <b>申请操作员</b>\n
		3.获取操作员申请：发送 <b>操作员申请</b>\n
		4.获取操作员列表：发送 <b>操作员列表</b>\n
		5.设置费率：发送 <b>设置费率 X.XX (例：设置费率 0.05)</b>\n
		6.设置汇率：发送 <b>设置汇率 币种X 汇率Y (设置汇率 PHP 7.8)</b>\n
		
		汇率 = 1RMB可以兑换的其他币种的值\n

		支持币种：人民币(RMB)、美元(USD)、披索(PHP)、马币(MYR)、泰铢(THB)、USDT(USDT)\n

		7.记录下发：发送 <b>下发 XXX Y币种(默认为人民币)</b> 或者 <b>-XXX Y币种(默认为人民币)</b>\n

		8.记录回款：发送 <b>回款 XXX Y币种(默认为人民币)</b> 或者 <b>+XXX Y币种(默认为人民币)</b>\n
	`
