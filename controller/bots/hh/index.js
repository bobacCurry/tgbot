...
const start = async (token,message_id,from,chat,text) => {
	...
	const r_text = `
		...
		1.添加管理：发送 <u>添加管理 <i>@用户名</i></u>\n

		2.删除管理：发送 <u>删除管理 <i>@用户名</i></u>\n

		⚠️添加的管理的行为必须在某一个群中进行\n
		
		3.设置费率：发送 <u>设置费率 <i>X.XX</i></u> (例：设置费率 0.05)\n
		
		4.设置汇率：发送 <u>设置汇率 <i>货币代码</i> <i>汇率</i></u> (例：设置汇率 PHP 7.8)\n
		...
		5.记录下发：发送 <u>下发 <i>数量</i> <i>货币代码</i></u>(例：下发 200000 PHP) 或者 <u>-<i>数量</i> <i>货币代码</i></u>(例：-200000 PHP)\n

		6.记录回款：发送 <u>回款 <i>数量</i> <i>货币代码</i></u>(例：回款 200000 PHP) 或者 <u>+<i>数量</i> <i>货币代码</i></u>(例：+200000 PHP)\n
